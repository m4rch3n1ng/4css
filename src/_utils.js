...
export function getConfig ( dirPath, outPath, options ) {
	const file = options.config ? getConfigFile(options.config) : {}
	...
	const dir = resolve(join(process.cwd(), file.dir || dirPath || "."))
	const out = file.out || outPath ? join(process.cwd(), file.out || outPath) : dir
...
function getConfigFile ( path ) {
	try {
		const actual = typeof path == "string"
			? path
		...
		const file = readFileSync(join(process.cwd(), actual)).toString()
...
export function terse ( css, sourceMap ) {
	const minifed = new CleanCSS({ sourceMap }).minify(css)
